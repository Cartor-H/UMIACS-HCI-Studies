...
function submit() {
    ...
    if (translatedMessage!="" && message!="" && to!="" && from!=""){
...
function onLoad(){
    ...
    if (watchingMode) {
        // let sse = new EventSource(`http://52.15.204.7:8080/stream?sender=${receiver}&to=${sender}&section=${document.title}`+
        // `&${(watchingMode?"&watchingMode":"")}`);
        let sendData = {
            flags   : flags          ,
            section : document.title ,
        ...
        sse.onmessage = (event) => {
            if(event.data!=null){
                ...
                //-----------------------------------------------------------------------------Local Response To New Message
                if(dataJSON["updateType"]=="message"){//dataJSON["message"]!=null){
                    const {message} = dataJSON;
                    // if (to == document.getElementById("senderID").value) {
                    let parsed = JSON.parse(message);
                    if(language=="Mandarin"){
                        addMessageRight(parsed["manMessage"],parsed["engMessage"])
                    } else {
                        addMessageRight(parsed["engMessage"],parsed["manMessage"])
                    }
                    // }
                }
...
function updateClock() {
    ...
    if (clockElement) {
        ...
        if (minutes>=30){
            ...
            if(language=="Mandarin"){
                document.getElementById("nav2").setAttribute("href","https://umdsurvey.umd.edu/jfe/form/SV_bK0czptuJFNY47Q")
            } else {
