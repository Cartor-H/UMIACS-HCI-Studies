...
promptSelected = JSON.stringify([
    {"role": "system", "content": "Generate two paraphrases for this message by following the instructions."+
    ...
    {"role": "system", "content": `
            ...
            1. The message is about two houses the person is considering to purchase.
            2. Use natural English expressions and sentence structure in your paraphrases.
            3. Change message's wording to make one paraphrase more casual and one more formal.
            4. The length of the paraphrase should be within 5 word difference with the user message.
...
promptUnselected =  JSON.stringify([
    {"role": "system", "content": "Generate two paraphrases for this message by following the instructions."},
    {"role": "user", "content": "_msg_"},
    {"role": "system", "content": `
...
function showParas(parasArray=null) {
    ...
    for (let i = 1; i <= numParas; i++) {
        ...
        <div class="position-relative">
            <div class="d-flex">
                <span class="flex-grow-1 flex-fill textarea-small dropdown-item btn-primary m-0 p-3"
                    onclick="//useParaphrase(${i})" id="optionMessage${i}">
                    ${(parasArray != null) ? parasArray[i-1].msg : ""}
                </span>
                <span class="position-absolute bottom-0 end-0" onclick="copyParaphrase(${i})"
                    id="copyBtn${i}" style="cursor: pointer;">📋</span>
            </div>
...
function showProgressBar() {
    ...
    document.getElementById('paraBtn').classList.toggle('d-none');
    fillProgressBar();
}
...
function fillProgressBar() {
    ...
    const increment = 100 / (2000 / interval); // Increment to fill up the progress bar in 2 seconds
...
function copyParaphrase(number) {
    let text = document.getElementById("optionMessage" + number).innerText;
    navigator.clipboard.writeText(text);


    // let elem = document.getElementById("copyBtn" + number);
    //
    // // Add the CSS class to trigger the transition effect
    // elem.classList.add("grow-shrink");
    //
    // // After a short delay, remove the class to make the effect temporary
    // setTimeout(() => {
    //     elem.classList.remove("grow-shrink");
...
function translateMessage() {
    ...
    if (message!="") {
        ...
        enablePara()
        document.getElementById("translatedMessage").setAttribute("contenteditable", "");
    }
}
...
function sendMessage(to, message) {
    fetch(`https://system-fai-im.info:8443/send-data?to=${to}&data=${message}&section=${document.title}`)
    ...
    if(agency){
        ...
        $.ajax({
            ...
            data: {mID: messageID, time: timeOfTranslation, senderID: from, source: message, keys: keyList,
                ...
                ParaphrasePushedTime       :  paraPushedTime,
                ParaphraseReturnTime       :  paraReturnTime,
...
function onLoad(){
    ...
    // let sse = new EventSource(`https://system-fai-im.info:8443/stream?sender=${sender}&to=${receiver}&section=${document.title}`+
    ...
    let sse = new EventSource(`https://system-fai-im.info:8443/connect?${new URLSearchParams(sendData).toString()}`);
    ...
    if (watchingMode) {
        // let sse = new EventSource(`https://system-fai-im.info:8443/connect?sender=${receiver}&to=${sender}&section=${document.title}`+
        // `&${(watchingMode?"&watchingMode":"")}`);
        // flags = encodeURIComponent(flags)
        let sendData = {
            flags   : flags          ,
...
function updateClock() {
    ...
    if (clockElement) {
        ...
        if (minutes>=30){
            ...
            if(language=="Mandarin"){
                document.getElementById("nav2").setAttribute("href","https://umdsurvey.umd.edu/jfe/form/SV_560vulnEWfB79LE")
            } else {
...
function notifyTypingHelper(to, status){
    ...
    fetch(`https://system-fai-im.info:8443/send-data?to=${to}&data=${status}&section=${document.title}`)
        .then(response => response.text())
        .then(result => {
            console.log(result);
        })
...
function copyText(text) {
    // Call the copy function in the Flash movie
